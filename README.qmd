...
6. Download and install these fonts (or install them from `misc/fonts` in this repository):
...
7. Run `targets::tar_make()` to run the full analysis pipeline. This will take ≈20 minutes the first time.
...
8. When the pipeline is all the way done, find the analysis notebook at `mountainous-mackerel/_site` and the manuscript and appendix files at `mountainous-mackerel/manuscript/output/`.
